...
| **F26** provider session limits and idle sessions | `RealtimeTransportOptions.IdleTimeoutSeconds` (default 30) ends a session in which neither side has spoken — the clock is reset by both, so a long answer never trips it — with reason `idle`. `MaxSessionDurationSeconds` (default 300) caps a single session however busy, with reason `max_duration`. Both clients say what happened and offer **Resume**. | **Extended:** the review proposed only the idle window, and in minutes. Idle alone does nothing about a session held open for hours, or a runaway page whose audio keeps the clock warm, so an absolute cap was added alongside it. |
