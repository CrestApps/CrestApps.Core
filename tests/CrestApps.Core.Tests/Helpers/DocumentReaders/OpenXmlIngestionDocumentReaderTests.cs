...
public sealed class OpenXmlIngestionDocumentReaderTests
{
    ...
    public async Task ReadAsync_WordDocument_ExtractsParagraphs()
    {
        ...
        var result = await _reader.ReadAsync(stream, "test.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", TestContext.Current.CancellationToken);
    ...
    public async Task ReadAsync_PowerPoint_ExtractsSlideText()
    {
        ...
        var result = await _reader.ReadAsync(stream, "test.pptx", "application/vnd.openxmlformats-officedocument.presentationml.presentation", TestContext.Current.CancellationToken);
        Assert.Single(result.Sections);
        Assert.Equal(2, result.Sections[0].Elements.Count);
    }
    ...
    public async Task ReadAsync_NonSeekableStream_ExtractsCorrectly()
    {
        ...
        var result = await _reader.ReadAsync(nonSeekable, "test.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", TestContext.Current.CancellationToken);
        Assert.Single(result.Sections);
        Assert.Single(result.Sections[0].Elements);
    ...
    [Fact]
    public async Task ReadAsync_FormFileStream_ExtractsCorrectly()
    {
        using var sourceStream = CreateWordDocument("FormFile content");
        var formFile = new FormFile(sourceStream, 0, sourceStream.Length, "files", "test.docx")
        {
        ...
        var result = await _reader.ReadAsync(formFileStream, "test.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", TestContext.Current.CancellationToken);
    ...
    private static MemoryStream CreateExcelWithBooleans(params bool[] values)
    {
        ...
        using (var doc = SpreadsheetDocument.Create(stream, SpreadsheetDocumentType.Workbook))
        {
            ...
            for (var i = 0; i < values.Length; i++)
            {
                ...
                row.AppendChild(new Cell { CellReference = cellRef, DataType = CellValues.Boolean, CellValue = new CellValue(values[i] ? "1" : "0"), });
            }
    ...
    private static MemoryStream CreateExcelWithMultipleSheets()
    {
        ...
        using (var doc = SpreadsheetDocument.Create(stream, SpreadsheetDocumentType.Workbook))
        {
            ...
            for (uint sheetNum = 1; sheetNum <= 2; sheetNum++)
            {
                ...
                row.AppendChild(new Cell { CellReference = "A1", DataType = CellValues.InlineString, InlineString = new InlineString(new DocumentFormat.OpenXml.Spreadsheet.Text($"Sheet{sheetNum} Data")), });
                sheetData.AppendChild(row);
                worksheetPart.Worksheet = new Worksheet(sheetData);
                sheets.AppendChild(new Sheet { Id = workbookPart.GetIdOfPart(worksheetPart), SheetId = sheetNum, Name = $"Sheet{sheetNum}", });
            }
        }
    ...
    private static MemoryStream CreateExcelWithMixedTypes()
    {
        ...
        using (var doc = SpreadsheetDocument.Create(stream, SpreadsheetDocumentType.Workbook))
        {
            ...
            // Shared string cell.
            row.AppendChild(new Cell { CellReference = "A1", DataType = CellValues.SharedString, CellValue = new CellValue("0"), });
            // Inline string cell.
            row.AppendChild(new Cell { CellReference = "B1", DataType = CellValues.InlineString, InlineString = new InlineString(new DocumentFormat.OpenXml.Spreadsheet.Text("InlineText")), });
            // Numeric cell (no DataType = default numeric).
            row.AppendChild(new Cell { CellReference = "C1", CellValue = new CellValue("42"), });
            // Boolean cell.
            row.AppendChild(new Cell { CellReference = "D1", DataType = CellValues.Boolean, CellValue = new CellValue("1"), });
            sheetData.AppendChild(row);
            worksheetPart.Worksheet = new Worksheet(sheetData);
            var sheets = workbookPart.Workbook.AppendChild(new Sheets());
            sheets.AppendChild(new Sheet { Id = workbookPart.GetIdOfPart(worksheetPart), SheetId = 1, Name = "Sheet1", });
    ...
    private static MemoryStream CreatePowerPoint(params string[] slideTexts)
    {
        ...
        using (var doc = PresentationDocument.Create(stream, PresentationDocumentType.Presentation))
        {
            ...
            foreach (var text in slideTexts)
            {
                ...
                slidePart.Slide = new P.Slide(new P.CommonSlideData(new P.ShapeTree(new P.NonVisualGroupShapeProperties(new P.NonVisualDrawingProperties { Id = 1, Name = "" }, new P.NonVisualGroupShapeDrawingProperties(), new P.ApplicationNonVisualDrawingProperties()), new P.GroupShapeProperties(new Drawing.TransformGroup()), new P.Shape(new P.NonVisualShapeProperties(new P.NonVisualDrawingProperties { Id = 2, Name = "TextBox" }, new P.NonVisualShapeDrawingProperties(), new P.ApplicationNonVisualDrawingProperties()), new P.ShapeProperties(), new P.TextBody(new Drawing.BodyProperties(), new Drawing.ListStyle(), new Drawing.Paragraph(new Drawing.Run(new Drawing.RunProperties { Language = "en-US" }, new Drawing.Text(text))))))));
