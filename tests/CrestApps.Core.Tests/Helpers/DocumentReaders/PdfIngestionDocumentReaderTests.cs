...
public sealed class PdfIngestionDocumentReaderTests
{
    ...
    public async Task ReadAsync_UnsupportedMediaType_ThrowsNotSupportedException()
    {
        ...
        await Assert.ThrowsAsync<NotSupportedException>(() => _reader.ReadAsync(stream, "test.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", TestContext.Current.CancellationToken));
    }
    ...
    {
        using var sourceStream = CreatePdfWithText("FormFile PDF");
        var formFile = new FormFile(sourceStream, 0, sourceStream.Length, "files", "test.pdf")
        {
        ...
        var result = await _reader.ReadAsync(formFileStream, "test.pdf", "application/pdf", TestContext.Current.CancellationToken);
