...
public sealed class IndexProfileTypeRulesTests
{
    ...
    private static IndexProfileController CreateController(TestSearchIndexProfileManager profileManager, TestRemoteSearchIndexManager remoteManager)
    {
        ...
        deploymentManager
            .Setup(manager => manager.GetAllBySlotAsync(AIDeploymentSlotNames.Embedding, It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .Returns((string _, string _, CancellationToken _) => ValueTask.FromResult<IEnumerable<AIDeployment>>([]));
        deploymentManager
