...
public sealed class IndexProfileTypeRulesTests
{
    ...
    private static IndexProfileController CreateController(TestSearchIndexProfileManager profileManager, TestRemoteSearchIndexManager remoteManager)
    {
        ...
        deploymentManager
            .Setup(manager => manager.GetByCapabilityAsync(AIDeploymentCapability.Embedding, It.IsAny<CancellationToken>()))
            .Returns((AIDeploymentCapability _, CancellationToken _) => ValueTask.FromResult<IEnumerable<AIDeployment>>([]));
        deploymentManager
