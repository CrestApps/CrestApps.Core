...
/// <summary>
/// Covers what the file source form does with a file server's connection settings.
/// </summary>
/// <remarks>
/// Two things here can only fail in production. A secret encrypted under a purpose the connector does not
/// decrypt with reads back as <see langword="null"/>, and the file source then connects with no credential
/// at all; and a secret the form did not resend has to survive the save, or editing the port silently
/// clears the password. Both are asserted against the connectors' own purpose constants rather than a local
/// copy.
/// </remarks>
public sealed class FileSourceViewModelTests
{
    ...
    /// <summary>
    /// Verifies that changing the connector takes the previous protocol's connection with it, so a record
    /// never carries a credential for a server it no longer reads.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Verifies that switching from a file server to a folder drops the connection and the remote folder,
    ...
    /// <summary>
    /// Verifies that a file-server file source keeps its folder settings alongside its connection.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Verifies that the connector the form posted is what the record is stored under. The source is the
    /// only thing telling a file source apart from a web crawler.
    /// </summary>
    [Fact]
