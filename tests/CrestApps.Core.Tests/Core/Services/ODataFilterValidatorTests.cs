...
public sealed class ODataFilterValidatorTests
{
    ...
    [Theory]
    [InlineData("category = 'documentation'")]
    [InlineData("invalidoperator test")]
    [InlineData("justtext")]
    ...
    [Theory]
    [InlineData("status eq 'unmatched")]
    ...
    [Theory]
    [InlineData("CATEGORY EQ 'test'")]
    [InlineData("status Ne 'active'")]
    [InlineData("value GT 10 AND status EQ 'ok'")]
