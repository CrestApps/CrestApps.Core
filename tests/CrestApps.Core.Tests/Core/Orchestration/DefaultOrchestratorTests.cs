...
public sealed class DefaultOrchestratorTests
{
    ...
    public async Task ScopeToolsAsync_AllToolsScoredUniformlyByRelevance()
    {
        var tools = new List<ToolRegistryEntry>
        {
            new()
            {
                Name = "userSelectedTool",
                Description = "A user selected content tool",
                Source = ToolRegistryEntrySource.Local
            },
            new()
            {
                Name = "createJiraTicket",
                Description = "Create a Jira ticket for issues",
                Source = ToolRegistryEntrySource.Local
            },
            new()
            {
    ...
    private static async Task<string> CollectStreamAsync(DefaultOrchestrator orchestrator, OrchestrationContext context)
    {
    ...
    private sealed class FakeToolRegistry : IToolRegistry
    {
        ...
        public Task<IReadOnlyList<ToolRegistryEntry>> GetAllAsync(AICompletionContext context, CancellationToken cancellationToken = default)
        {
            return Task.FromResult(_entries);
        }

        public Task<IReadOnlyList<ToolRegistryEntry>> SearchAsync(string query, int topK, AICompletionContext context, CancellationToken cancellationToken = default)
        {
            return Task.FromResult<IReadOnlyList<ToolRegistryEntry>>(_entries.Take(topK).ToList());
        }
    }
