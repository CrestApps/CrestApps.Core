...
public sealed class PreemptiveRagOrchestrationHandlerTests
{
    ...
    private static PreemptiveRagOrchestrationHandler CreateHandler(IPreemptiveRagHandler[] handlers, bool enablePreemptiveRag)
    {
        var templateService = new FakeAITemplateService();
        var deploymentManager = new Mock<IAIDeploymentManager>();
        var queryProvider = new PreemptiveSearchQueryProvider(new NullAIClientFactory(), deploymentManager.Object, templateService, Mock.Of<IServiceProvider>(), NullLogger<PreemptiveSearchQueryProvider>.Instance);
    ...
    private static Mock<IPreemptiveRagHandler> CreateFakeRagHandler(bool canHandle, bool addDataSourceRefs = false)
    {
        ...
        mock.Setup(h => h.CanHandleAsync(It.IsAny<OrchestrationContextBuiltContext>())).ReturnsAsync(canHandle);
        if (addDataSourceRefs)
        {
            mock.Setup(h => h.HandleAsync(It.IsAny<PreemptiveRagContext>())).Callback<PreemptiveRagContext>(ctx =>
            {
    ...
    private static OrchestrationContext CreateOrchestrationContext(string userMessage = "", bool disableTools = false)
    {
