...
/// while this orchestrator bridges the tool registry and MCP connections
...
public sealed class CopilotOrchestrator : IOrchestrator
{
    ...
    public async IAsyncEnumerable<ChatResponseUpdate> ExecuteStreamingAsync(OrchestrationContext context, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        foreach (var entry in allTools)
        {
            ...
            try
            {
                var aiTool = context.ServiceProvider is not null ? await entry.CreateAsync(context.ServiceProvider) : null;
        ...
        {
            _logger.LogDebug("CopilotOrchestrator: Resolved {ToolCount} tool(s): [{Tools}]", tools.Count, string.Join(", ", tools.Select(t => t.Name)));
        }
        ...
        if (context.Properties.TryGetValue(nameof(CopilotSessionMetadata), out var metadataObj) && metadataObj is CopilotSessionMetadata md)
        {
            metadata = md;
    ...
    private async Task<CopilotClientOptions> BuildClientOptionsAsync(OrchestrationContext context, CopilotSessionMetadata metadata, CopilotOptions settings)
    {
        ...
        else
        {
            ...
            if (metadata is not null && !string.IsNullOrEmpty(metadata.ProtectedAccessToken))
            {
                ...
                {
                    _logger.LogDebug("CopilotOrchestrator: Using profile-level credential from {Username}", metadata.GitHubUsername);
                }
            }

            // Fall back to the current user's credentials via ICopilotCredentialStore.
                ...
                if (user?.Identity?.IsAuthenticated == true)
                {
                    ...
                    if (!string.IsNullOrEmpty(userId))
                    {
                        ...
                        if (credential is not null && !string.IsNullOrEmpty(credential.ProtectedAccessToken))
                        {
                            try
                            {
                                var protector = _dataProtectionProvider.CreateProtector(TokenProtectorPurpose);
                                accessToken = protector.Unprotect(credential.ProtectedAccessToken);
                                if (!string.IsNullOrEmpty(accessToken) && _logger.IsEnabled(LogLevel.Debug))
                                {
                                    _logger.LogDebug("CopilotOrchestrator: Using user-level credential for user {UserId}", userId);
                                }
                            }
                            catch (Exception ex)
                            {
                                _logger.LogError(ex, "CopilotOrchestrator: Failed to unprotect access token for user {UserId}.", userId);
                            }
                        }
                    }
                }
            ...
            {
                _logger.LogWarning("CopilotOrchestrator: No valid GitHub access token found. " + "The session may fail without authentication.");
            }
        }
    ...
    private static bool IsConfigured(CopilotOptions settings)
    {
        ...
        {
            CopilotAuthenticationType.GitHubOAuth => !string.IsNullOrWhiteSpace(settings.ClientId) && !string.IsNullOrWhiteSpace(settings.ClientSecret),
            CopilotAuthenticationType.ApiKey => !string.IsNullOrWhiteSpace(settings.ProviderType) && !string.IsNullOrWhiteSpace(settings.BaseUrl) && !string.IsNullOrWhiteSpace(settings.DefaultModel) && (!string.Equals(settings.ProviderType, "azure", StringComparison.OrdinalIgnoreCase) || (!string.IsNullOrWhiteSpace(settings.AzureApiVersion) && !string.IsNullOrWhiteSpace(settings.ApiKey))),
            _ => false,
        };
    }
    ...
    /// </summary>
    private void ConfigureByokProvider(SessionConfig sessionConfig, CopilotOptions settings)
    {
        ...
        // CopilotOptions stores unprotected secrets, so use ApiKey directly.
        ...
        if (string.Equals(settings.ProviderType, "azure", StringComparison.OrdinalIgnoreCase) && !string.IsNullOrEmpty(settings.AzureApiVersion))
        {
        ...
        if (_logger.IsEnabled(LogLevel.Debug))
        {
            _logger.LogDebug("CopilotOrchestrator: Configured BYOK provider type='{Type}', baseUrl='{BaseUrl}', model='{Model}'.", providerConfig.Type, providerConfig.BaseUrl, sessionConfig.Model);
        }
    }
    ...
    private async Task<string> RunCopilotSessionAsync(CopilotClientOptions clientOptions, SessionConfig sessionConfig, OrchestrationContext context, CancellationToken cancellationToken)
    {
        ...
        using var subscription = session.On(ev =>
        {
            ...
            else if (ev is SessionErrorEvent errorEvent)
            {
                ...
                _logger.LogError("CopilotOrchestrator: Session error - {ErrorType}: {Message} (StatusCode: {StatusCode})", data?.ErrorType, data?.Message, data?.StatusCode);
    ...
    /// </summary>
    private async Task ConfigureMcpServersAsync(OrchestrationContext context, SessionConfig sessionConfig)
    {
    ...
    private sealed class ServiceInjectedAIFunction : AIFunction
    {
        ...
        public override IReadOnlyDictionary<string, object> AdditionalProperties
        {
            get
            {
                return _inner.AdditionalProperties;
            }
        }
        ...
        public override JsonSerializerOptions JsonSerializerOptions
        {
            get
            {
                return _inner.JsonSerializerOptions;
            }
        }
        ...
        public override MethodInfo UnderlyingMethod
        {
            get
            {
                return _inner.UnderlyingMethod;
            }
        }
