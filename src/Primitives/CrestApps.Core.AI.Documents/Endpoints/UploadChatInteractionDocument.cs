...
public static partial class AIChatDocumentEndpoints
{
        ...
        [FromServices] IStringLocalizerFactory localizerFactory)
    {
        ...
        foreach (var file in files)
        {
            ...
            var result = await ProcessFileAsync(file, interaction.ItemId, AIReferenceTypes.Document.ChatInteraction, documentOptions.Value, documentProcessingService, embeddingGenerator, documentStore, chunkStore, fileStore, logger, S);
            if (!result.Success)
            {
