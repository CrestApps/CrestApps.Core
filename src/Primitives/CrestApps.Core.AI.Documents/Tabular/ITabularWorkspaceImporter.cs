...
/// format-specific fast path is available. A single source may contain multiple worksheets, each of
/// which is imported into its own table.
/// </summary>
public interface ITabularWorkspaceImporter
{
    /// <summary>
    /// Imports the supplied source file directly into the workspace, creating one table per worksheet.
    /// </summary>
    ...
    /// handle the source.
    /// </returns>
    Task<IReadOnlyList<TabularWorkspaceImportResult>> ImportAsync(
        Stream source,
        string fileName,
        string contentType,
