...
#pragma warning restore MEAI001
{
    ...
    public AzureSpeechServiceSpeechToTextClient(Uri endpoint, AzureAuthenticationType authType, string apiKey, string identityId, TimeProvider timeProvider, ILogger logger)
    {
...
#pragma warning disable MEAI001
    public async Task<SpeechToTextResponse> GetTextAsync(Stream audioSpeechStream, SpeechToTextOptions options = null, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogTrace("[STT:{TraceId}] +{Elapsed}ms GetTextAsync START. Language={Language}, AuthType={AuthType}", traceId, sw.ElapsedMilliseconds, language, _authType);
        }
        ...
        if (_logger.IsEnabled(LogLevel.Trace))
        {
            _logger.LogTrace("[STT:{TraceId}] +{Elapsed}ms Audio push complete. TotalBytes={TotalBytes}. Starting RecognizeOnceAsync...", traceId, sw.ElapsedMilliseconds, totalBytes);
        }

        var result = await recognizer.RecognizeOnceAsync();
        if (_logger.IsEnabled(LogLevel.Trace))
        {
            _logger.LogTrace("[STT:{TraceId}] +{Elapsed}ms RecognizeOnceAsync returned. Reason={Reason}", traceId, sw.ElapsedMilliseconds, result.Reason);
        }
        ...
        if (result.Reason == ResultReason.NoMatch)
        {
            ...
            {
                _logger.LogDebug("[STT:{TraceId}] +{Elapsed}ms NoMatch from {TotalBytes} bytes.", traceId, sw.ElapsedMilliseconds, totalBytes);
            }
        }
        else if (result.Reason == ResultReason.Canceled)
        {
            var cancellation = CancellationDetails.FromResult(result);
            _logger.LogWarning("[STT:{TraceId}] +{Elapsed}ms CANCELED. Reason={Reason}, ErrorCode={ErrorCode}, Details={ErrorDetails}", traceId, sw.ElapsedMilliseconds, cancellation.Reason, cancellation.ErrorCode, cancellation.ErrorDetails);
        }
    ...
    public async IAsyncEnumerable<SpeechToTextResponseUpdate> GetStreamingTextAsync(Stream audioSpeechStream, SpeechToTextOptions options = null, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        if (_logger.IsEnabled(LogLevel.Trace))
        {
            _logger.LogTrace("[STT:{TraceId}] +{Elapsed}ms GetStreamingTextAsync START. Language={Language}, AuthType={AuthType}", traceId, sw.ElapsedMilliseconds, language, _authType);
        }
        ...
        recognizer.Recognizing += (_, e) =>
        {
            if (!string.IsNullOrEmpty(e.Result.Text))
            {
                if (_logger.IsEnabled(LogLevel.Trace))
                {
                    _logger.LogTrace("[STT:{TraceId}] +{Elapsed}ms PARTIAL: '{Text}'", traceId, sw.ElapsedMilliseconds, e.Result.Text);
                }

                channel.Writer.TryWrite(new SpeechToTextResponseUpdate(e.Result.Text) { AdditionalProperties = new AdditionalPropertiesDictionary { ["isPartial"] = true, }, });
            }
        };
        ...
        recognizer.SessionStarted += (_, e) =>
        {
            if (_logger.IsEnabled(LogLevel.Trace))
            {
                _logger.LogTrace("[STT:{TraceId}] +{Elapsed}ms SessionStarted. SdkSessionId={SessionId}", traceId, sw.ElapsedMilliseconds, e.SessionId);
            }
        };
        recognizer.SessionStopped += (_, e) =>
        {
            if (_logger.IsEnabled(LogLevel.Trace))
            {
                _logger.LogTrace("[STT:{TraceId}] +{Elapsed}ms SessionStopped. SdkSessionId={SessionId}", traceId, sw.ElapsedMilliseconds, e.SessionId);
            }
        ...
        recognizer.Canceled += (_, e) =>
        {
            ...
            {
                _logger.LogWarning("[STT:{TraceId}] +{Elapsed}ms CANCELED with error. ErrorCode={ErrorCode}, Details={ErrorDetails}", traceId, sw.ElapsedMilliseconds, e.ErrorCode, e.ErrorDetails);
                errorCts.Cancel();
                pushStream.Close();
            ...
            else
            {
                if (_logger.IsEnabled(LogLevel.Trace))
                {
                    _logger.LogTrace("[STT:{TraceId}] +{Elapsed}ms CANCELED (non-error). Reason={Reason}", traceId, sw.ElapsedMilliseconds, e.Reason);
                }
    ...
    private async Task PushAudioToStreamAsync(string traceId, Stopwatch sw, Stream audioSpeechStream, PushAudioInputStream pushStream, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            while ((bytesRead = await audioSpeechStream.ReadAsync(buffer.AsMemory(0, buffer.Length), cancellationToken)) > 0)
            {
                ...
                if (_logger.IsEnabled(LogLevel.Trace))
                {
                    _logger.LogTrace("[STT:{TraceId}] +{Elapsed}ms PushChunk #{ChunkCount}: {BytesRead} bytes (total={TotalBytes})", traceId, sw.ElapsedMilliseconds, chunkCount, bytesRead, totalBytes);
                }
            }
        ...
        catch (OperationCanceledException)when (cancellationToken.IsCancellationRequested)
        {
            if (_logger.IsEnabled(LogLevel.Trace))
            {
                _logger.LogTrace("[STT:{TraceId}] +{Elapsed}ms PushAudioToStream CANCELED after {TotalBytes} bytes.", traceId, sw.ElapsedMilliseconds, totalBytes);
            }
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "[STT:{TraceId}] +{Elapsed}ms PushAudioToStream ERROR after {TotalBytes} bytes.", traceId, sw.ElapsedMilliseconds, totalBytes);
        }
        finally
        {
            pushStream.Close();
...
#pragma warning restore MEAI001
    ...
    private async Task<SpeechConfig> CreateSpeechConfigAsync(string language, CancellationToken cancellationToken)
    {
        ...
        if (_authType == AzureAuthenticationType.ApiKey)
        {
            if (_logger.IsEnabled(LogLevel.Debug))
            {
                _logger.LogDebug("Using endpoint-based SDK configuration for API key authentication. Endpoint: {Endpoint}, Region: {Region}", _endpoint, _region ?? "(unknown)");
            }
        ...
        else
        {
            if (_logger.IsEnabled(LogLevel.Debug))
            {
                _logger.LogDebug("Could not extract region from endpoint '{Endpoint}'. Falling back to endpoint-based SDK configuration.", _endpoint);
            }
        ...
        if (_logger.IsEnabled(LogLevel.Trace))
        {
            _logger.LogTrace("SpeechConfig created. Language: {Language}, AuthType: {AuthType}, Endpoint: {Endpoint}, Region: {Region}", language, _authType, _endpoint, _region ?? "(from endpoint)");
        }
    ...
    private async Task<SpeechConfig> CreateRegionBasedConfigAsync(string region, CancellationToken cancellationToken)
    {
        if (_logger.IsEnabled(LogLevel.Debug))
        {
            var maskedKey = !string.IsNullOrEmpty(_apiKey) ? "[Redacted]" : "(not set)";
            _logger.LogDebug("Creating region-based SpeechConfig. Region: {Region}, AuthType: {AuthType}, ApiKey: {MaskedKey}, IdentityId: {IdentityId}", region, _authType, maskedKey, string.IsNullOrEmpty(_identityId) ? "(system)" : _identityId);
        }

        return _authType switch
        {
            AzureAuthenticationType.ApiKey => SpeechConfig.FromSubscription(_apiKey, region),
            AzureAuthenticationType.ManagedIdentity or AzureAuthenticationType.Default => SpeechConfig.FromAuthorizationToken(await GetAuthorizationTokenAsync(cancellationToken), region),
            _ => throw new NotSupportedException($"Authentication type '{_authType}' is not supported for Azure Speech."),
        };
    }
    ...
    private async Task<SpeechConfig> CreateEndpointBasedConfigAsync(CancellationToken cancellationToken)
    {
        return _authType switch
        {
            AzureAuthenticationType.ApiKey => SpeechConfig.FromEndpoint(_endpoint, _apiKey),
            AzureAuthenticationType.ManagedIdentity or AzureAuthenticationType.Default => CreateEndpointConfigWithToken(await GetAuthorizationTokenAsync(cancellationToken)),
            _ => throw new NotSupportedException($"Authentication type '{_authType}' is not supported for Azure Speech."),
        };
    }
    ...
    private async Task<string> GetAuthorizationTokenAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                AzureAuthenticationType.ManagedIdentity => string.IsNullOrEmpty(_identityId) ? new ManagedIdentityCredential(ManagedIdentityId.SystemAssigned) : new ManagedIdentityCredential(ManagedIdentityId.FromUserAssignedClientId(_identityId)),
                _ => new DefaultAzureCredential(),
            };
            var tokenResult = await credential.GetTokenAsync(new TokenRequestContext([CognitiveServicesScope]), cancellationToken);
...
#pragma warning restore MEAI001
    {
        if (options?.AdditionalProperties is not null && options.AdditionalProperties.TryGetValue("audioFormat", out var formatValue) && formatValue is string formatString && !string.IsNullOrWhiteSpace(formatString))
        {
