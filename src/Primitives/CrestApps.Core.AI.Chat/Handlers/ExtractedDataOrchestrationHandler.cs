...
public sealed class ExtractedDataOrchestrationHandler : IOrchestrationContextBuilderHandler
{
    ...
    public async Task BuiltAsync(OrchestrationContextBuiltContext context)
    {
        ...
        var collectedFields = settings.DataExtractionEntries.Where(entry => session.ExtractedData.TryGetValue(entry.Name, out var state) && state?.Values.Count > 0).Select(entry => new { entry.Name, entry.Description, Values = session.ExtractedData[entry.Name].Values, entry.AllowMultipleValues, entry.IsUpdatable, }).ToList();
