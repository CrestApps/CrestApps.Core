    ...
    public virtual async Task StartConversation(string itemId, IAsyncEnumerable<string> audioChunks, string audioFormat = null, string language = null)
    {
        ...
        try
        {
            await ExecuteInScopeAsync(async services =>
            {
                ...
                var speechToTextDeployment = await deploymentManager.ResolveOrDefaultAsync(AIDeploymentCapability.SpeechToText);
                ...
                var textToSpeechDeployment = await deploymentManager.ResolveOrDefaultAsync(AIDeploymentCapability.TextToSpeech);
