...
public abstract class ChatInteractionHubBase : Hub<IChatInteractionHubClient>
{
    ...
    protected ChatInteractionHubBase(ICatalogManager<ChatInteraction> interactionManager, IChatInteractionPromptStore promptStore, IOrchestrationContextBuilder orchestrationContextBuilder, IOrchestratorResolver orchestratorResolver, IEnumerable<IChatInteractionSettingsHandler> settingsHandlers, IServiceProvider serviceProvider, TimeProvider timeProvider, ILogger logger)
    {
    ...
    /// resolves <see cref="IStoreCommitter"/> from the hub's service scope and calls
    /// <see cref="IStoreCommitter.CommitAsync"/>. Override only when your store does not
    /// register <see cref="IStoreCommitter"/> or requires a different commit strategy.
    /// </summary>
    protected virtual async Task CommitChangesAsync()
    {
        if (Context is null)
        {
            ...
            {
                Logger.LogDebug("ChatInteractionHubBase Context is null, cannot commit staged store changes for connection '{ConnectionId}'.", Context.ConnectionId);
            }

            return;
        }

        var committer = _serviceProvider.GetService<IStoreCommitter>();
        ...
        {
            Logger.LogDebug("ChatInteractionHubBase IStoreCommitter is null, cannot commit staged store changes for connection '{ConnectionId}'.", Context.ConnectionId);
        }
    }
    ...
    public virtual async Task LoadInteraction(string itemId)
    {
        ...
        await Clients.Caller.LoadInteraction(new { interaction.ItemId, interaction.Title, interaction.ConnectionName, interaction.ChatDeploymentName, ChatDeploymentId = interaction.ChatDeploymentName, Messages = prompts.Select(message => new AIChatResponseMessageDetailed { Id = message.ItemId, Role = message.Role.Value, IsGeneratedPrompt = message.IsGeneratedPrompt, Title = message.Title, Content = message.Text, References = message.References, }) });
    }
    ...
    /// </summary>
    protected virtual void CollectPreemptiveReferences(OrchestrationContext context, Dictionary<string, AICompletionReference> references, HashSet<string> contentItemIds)
    {
    }
    ...
    /// </summary>
    protected virtual void CollectToolReferences(Dictionary<string, AICompletionReference> references, HashSet<string> contentItemIds)
    {
    }
    ...
    /// </summary>
    protected virtual Task OnAssistantPromptCreatedAsync(ChatInteractionPrompt prompt, HashSet<string> contentItemIds)
    {
        return Task.CompletedTask;
    }

    protected virtual async Task HandlePromptAsync(ChannelWriter<CompletionPartialMessage> writer, string itemId, string prompt, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var orchestratorContext = await OrchestrationContextBuilder.BuildAsync(interaction, ctx =>
            {
                ...
                ctx.ConversationHistory = existingPrompts.Where(x => !x.IsGeneratedPrompt).Select(p => new ChatMessage(p.Role, p.Text)).ToList();
            });
    ...
    protected virtual void ApplyCoreSettings(ChatInteraction interaction, JsonElement settings)
    {
        ...
        interaction.ChatDeploymentName = JsonHelper.GetString(settings, "deploymentName") ?? JsonHelper.GetString(settings, "deploymentId");
