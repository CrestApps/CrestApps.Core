...
/// <summary>
/// The authoritative catalog handler for <see cref="AIToolInstance"/> entries. It maps incoming JSON onto
...
internal sealed class AIToolInstanceCatalogHandler : CatalogEntryHandlerBase<AIToolInstance>
{
    ...
    /// <param name="aiOptions">The AI options used to validate the selected tool source.</param>
    /// <param name="serviceProvider">The service provider used to resolve the instance catalog for duplicate-name checks.</param>
    ...
    /// <param name="cancellationToken">The cancellation token.</param>
    public override async Task ValidatingAsync(ValidatingContext<AIToolInstance> context, CancellationToken cancellationToken = default)
    {
