...
internal sealed class AIMemoryOrchestrationHandler : IOrchestrationContextBuilderHandler
{
    ...
    public async Task BuiltAsync(OrchestrationContextBuiltContext context)
    {
        ...
        if (_logger.IsEnabled(LogLevel.Debug))
        {
            _logger.LogDebug("Enabled memory orchestration for {ResourceType} and exposed tools: {Tools}.", context.Resource.GetType().Name, string.Join(", ", memoryTools.Select(tool => tool.Name)));
        }
    }
}
