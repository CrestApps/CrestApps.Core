...
public sealed class DefaultChatResponseHandlerResolver : IChatResponseHandlerResolver
{
    ...
    public IChatResponseHandler Resolve(string handlerName = null, ChatMode chatMode = ChatMode.TextInput)
    {
        ...
        {
            if (!string.IsNullOrWhiteSpace(handlerName) && !string.Equals(handlerName, ChatResponseHandlerNames.AI, StringComparison.OrdinalIgnoreCase))
            {
                _logger.LogWarning("Chat response handler '{HandlerName}' was requested but conversation mode requires the AI handler. Falling back to AI.", handlerName);
            }
    ...
    private IChatResponseHandler ResolveAIHandler()
    {
        ...
        throw new InvalidOperationException("The default AI chat response handler is not registered. " + "Ensure CrestApps AI services are properly configured.");
    }
}