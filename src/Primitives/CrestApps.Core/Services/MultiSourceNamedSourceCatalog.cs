...
public abstract class MultiSourceNamedSourceCatalog<T> : INamedSourceCatalog<T>
    where T : INameAwareModel, ISourceAwareModel
{
    private readonly IEnumerable<INamedSourceCatalogSource<T>> _sources;
    private readonly IWritableNamedSourceCatalogSource<T> _writableSource;
    ...
    protected MultiSourceNamedSourceCatalog(IEnumerable<INamedSourceCatalogSource<T>> sources)
    {
        ...
        _writableSource = sources
            .OfType<IWritableNamedSourceCatalogSource<T>>()
    ...
    private void EnsureWritableSource()
    {
        if (_writableSource is null)
        {
            throw new InvalidOperationException(
                ...
                Register an {nameof(IWritableNamedSourceCatalogSource<>)} implementation to enable write operations.
                """);
        }
    }
}
