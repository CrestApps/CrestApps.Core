...
/// <summary>
/// Markdown parser that extracts front matter metadata from template files.
...
/// </example>
public sealed class DefaultMarkdownTemplateParser : ITemplateParser
{
    ...
    private static void ParseFrontMatter(ReadOnlySpan<char> frontMatter, TemplateMetadata metadata)
    {
    ...
    private static void SetMetadataValue(TemplateMetadata metadata, string key, string value)
    {
        if (string.Equals(key, nameof(TemplateMetadata.Title), StringComparison.OrdinalIgnoreCase))
        {
            metadata.Title = value;
        }
        else if (string.Equals(key, nameof(TemplateMetadata.Description), StringComparison.OrdinalIgnoreCase))
        {
            metadata.Description = value;
        }
        else if (string.Equals(key, nameof(TemplateMetadata.IsListable), StringComparison.OrdinalIgnoreCase))
        {
        ...
        else if (string.Equals(key, nameof(TemplateMetadata.Category), StringComparison.OrdinalIgnoreCase))
        {
            metadata.Category = value;
        }
        else if (string.Equals(key, nameof(TemplateMetadata.Parameters), StringComparison.OrdinalIgnoreCase))
        {
    ...
    /// </summary>
    private static List<TemplateParameterDescriptor> ParseParameterDescriptors(string value)
    {
        var parameters = new List<TemplateParameterDescriptor>();
        ...
        foreach (var rawLine in value.Split('\n'))
        {
            ...
            if (name.Length > 0)
            {
                parameters.Add(new TemplateParameterDescriptor
                {
                    Name = name,
