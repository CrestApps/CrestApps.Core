...
internal sealed class FindAgentForTaskFunction : AIFunction
{
    ...
    public override string Description
    {
        get
        {
            return "Finds the most relevant AI agents for a specific task using keyword and semantic matching. Searches both local agents and remote A2A agents.";
        }
    }
    ...
    protected override async ValueTask<object> InvokeCoreAsync(AIFunctionArguments arguments, CancellationToken cancellationToken)
    {
        ...
        if (!arguments.TryGetFirstString("taskDescription", out var taskDescription) || string.IsNullOrWhiteSpace(taskDescription))
        {
        ...
        try
        {
            ...
            if (localProfiles is not null)
            {
                foreach (var profile in localProfiles)
                {
                    ...
                    scoredAgents.Add((new { name = profile.DisplayText ?? profile.Name, id = profile.Name, description = profile.Description, source = "local", relevanceScore = Math.Round(score, 3), }, score));
                }
            }
        }
        catch (Exception ex)
        ...
        try
        {
            ...
            foreach (var connection in connections)
            {
                ...
                try
                {
                    var agentCard = await agentCardCache.GetAgentCardAsync(connection.ItemId, connection, cancellationToken);
                    ...
                    foreach (var skill in agentCard.Skills)
                    {
                        ...
                        var score = ScoreRelevance(queryTokens, tokenizer.Tokenize(text));
                        scoredAgents.Add((new { name = skill.Name ?? skill.Id, id = skill.Id, description = skill.Description ?? agentCard.Description, source = "remote", host = connection.DisplayText, tags = skill.Tags, relevanceScore = Math.Round(score, 3), }, score));
                    }
                }
                catch (Exception ex)
                {
