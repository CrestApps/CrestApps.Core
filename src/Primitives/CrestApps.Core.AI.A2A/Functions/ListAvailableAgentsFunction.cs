...
internal sealed class ListAvailableAgentsFunction : AIFunction
{
    ...
    public override string Description
    {
        get
        {
            return "Lists all available AI agents, including local agents and remote agents from connected A2A hosts. Returns their names, descriptions, and capabilities.";
        }
    }
    ...
    protected override async ValueTask<object> InvokeCoreAsync(AIFunctionArguments arguments, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (localProfiles is not null)
            {
                ...
                {
                    agents.Add(new { name = profile.DisplayText ?? profile.Name, id = profile.Name, description = profile.Description, source = "local", });
                }
            }
        }
        catch (Exception ex)
        ...
        try
        {
            ...
            foreach (var connection in connections)
            {
                ...
                try
                {
                    ...
                    {
                        agents.Add(new { name = skill.Name ?? skill.Id, id = skill.Id, description = skill.Description ?? agentCard.Description, source = "remote", host = connection.DisplayText, tags = skill.Tags, });
                    }
                }
                catch (Exception ex)
                {
