...
internal sealed class DefaultA2AConnectionAuthService : IA2AConnectionAuthService
{
    ...
    private void BuildBasicHeaders(A2AConnectionMetadata metadata, IDataProtector protector, Dictionary<string, string> headers)
    {
        ...
        var password = !string.IsNullOrEmpty(metadata.BasicPassword) ? Unprotect(protector, metadata.BasicPassword) : string.Empty;
    ...
    private async Task BuildOAuth2ClientCredentialsHeadersAsync(A2AConnectionMetadata metadata, IDataProtector protector, Dictionary<string, string> headers, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var token = await AcquireTokenAsync("cc", metadata.OAuth2TokenEndpoint, metadata.OAuth2ClientId, metadata.OAuth2Scopes, new Dictionary<string, string> { ["grant_type"] = "client_credentials", ["client_id"] = metadata.OAuth2ClientId, ["client_secret"] = clientSecret, }, cancellationToken);
    ...
    private async Task BuildOAuth2PrivateKeyJwtHeadersAsync(A2AConnectionMetadata metadata, IDataProtector protector, Dictionary<string, string> headers, CancellationToken cancellationToken)
    {
        ...
        var assertion = CreateClientAssertion(metadata.OAuth2TokenEndpoint, metadata.OAuth2ClientId, privateKey, metadata.OAuth2KeyId);
        try
        {
            var token = await AcquireTokenAsync("pkjwt", metadata.OAuth2TokenEndpoint, metadata.OAuth2ClientId, metadata.OAuth2Scopes, new Dictionary<string, string> { ["grant_type"] = "client_credentials", ["client_id"] = metadata.OAuth2ClientId, ["client_assertion_type"] = "urn:ietf:params:oauth:client-assertion-type:jwt-bearer", ["client_assertion"] = assertion, }, cancellationToken);
            headers["Authorization"] = $"Bearer {token}";
        }
        catch (Exception ex)
        {
    ...
    private async Task BuildOAuth2MtlsHeadersAsync(A2AConnectionMetadata metadata, IDataProtector protector, Dictionary<string, string> headers, CancellationToken cancellationToken)
    {
        ...
        var certPassword = !string.IsNullOrEmpty(metadata.OAuth2ClientCertificatePassword) ? Unprotect(protector, metadata.OAuth2ClientCertificatePassword) : null;
        try
        {
            ...
            var cert = string.IsNullOrEmpty(certPassword) ? X509CertificateLoader.LoadPkcs12(certBytes, null) : X509CertificateLoader.LoadPkcs12(certBytes, certPassword);
            using (cert)
            {
    ...
    private async Task<string> AcquireTokenAsync(string grantType, string tokenEndpoint, string clientId, string scopes, Dictionary<string, string> parameters, CancellationToken cancellationToken)
    {
    ...
    private async Task<string> SendTokenRequestAsync(HttpClient httpClient, string tokenEndpoint, Dictionary<string, string> parameters, string cacheKey, CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            _logger.LogError("OAuth2 token request to '{TokenEndpoint}' failed with status {StatusCode}: {ErrorBody}", tokenEndpoint, response.StatusCode, errorBody);
            throw new HttpRequestException($"OAuth2 token request failed with status {response.StatusCode}.");
        }
    ...
    {
        return $"a2a_oauth2_{grantType}_{tokenEndpoint}_{clientId}_{scopes}";
    }
