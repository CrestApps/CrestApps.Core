...
namespace CrestApps.Core.Startup.Shared.ViewModels;

/// <summary>
/// One editable row in the tool instance parameter editor. Shared by both sample hosts so the MVC and
/// Blazor editors stay in step, and so the mapping to and from
/// <see cref="AIToolInstanceParameter"/> — including the "leave a secret blank to keep it" rule — is
/// written once.
/// </summary>
public sealed class AIToolInstanceParameterViewModel
{
    /// <summary>
    /// Gets or sets a stable per-row key. Used by the Blazor editor to key rendered rows so inputs keep
    /// their state when a row is added, removed, or reordered.
    /// </summary>
    ...
    /// <summary>
    /// Gets or sets the default value, or the pinned value for a fixed parameter. Held as text because
    /// the editor is a form; it is converted to the declared type when the instance is saved.
    /// </summary>
    public string DefaultValue { get; set; }

    /// <summary>
    /// Gets or sets the pinned value for a fixed parameter. Held separately from
    /// <see cref="DefaultValue"/> so the two editor fields can be shown and posted independently.
    /// </summary>
    ...
    /// <returns>The editor rows.</returns>
    ...
    /// <summary>
    /// Converts the editor rows back into declared parameters, carrying forward any stored secret the
    /// user left blank.
    /// </summary>
    /// <param name="rows">The submitted editor rows.</param>
    /// <param name="existing">The parameters currently stored on the instance.</param>
    /// <param name="protect">A delegate that protects a newly entered secret value.</param>
    /// <returns>The parameters to persist.</returns>
