...
public sealed class FileSystemFileStore
{
    ...
    {
        var filePath = GetSafePath(fileName);
        if (File.Exists(filePath)) { File.Delete(filePath); return Task.FromResult(true); }
    ...
    private static string NormalizeRelativePath(string fileName)
    {
        ...
        if (segments.Length == 0) throw new ArgumentException("The file name contains an invalid path.");
