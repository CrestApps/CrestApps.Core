...
public sealed class ChatInteractionHub : ChatInteractionHubBase
{
    ...
    protected override void CollectPreemptiveReferences(OrchestrationContext context, Dictionary<string, AICompletionReference> references, HashSet<string> contentItemIds)
    {
        _citationCollector.CollectPreemptiveReferences(context, references, contentItemIds);
    }

    protected override void CollectToolReferences(Dictionary<string, AICompletionReference> references, HashSet<string> contentItemIds)
    {
    ...
    public async Task HandleNotificationAction(string itemId, string notificationType, string actionName)
    {
        ...
        try
        {
            ...
            await handler.HandleAsync(new ChatNotificationActionContext { SessionId = itemId, NotificationType = notificationType, ActionName = actionName, ChatType = ChatContextType.ChatInteraction, ConnectionId = Context.ConnectionId, Services = scope.ServiceProvider, });
        }
        catch (Exception ex)
        {
    ...
    public async Task StartConversation(string itemId, IAsyncEnumerable<string> audioChunks, string audioFormat = null, string language = null)
    {
        ...
        try
        {
            await RunConversationLoopAsync(itemId, audioChunks, audioFormat, string.IsNullOrWhiteSpace(language) ? "en-US" : language, speechToTextClient, textToSpeechClient, deploymentDefaults.DefaultTextToSpeechVoiceId, conversationCts.Token);
        }
    ...
    public async Task SendAudioStream(string itemId, IAsyncEnumerable<string> audioChunks, string audioFormat = null, string language = null)
    {
        ...
        {
            using var speechToTextClient = await _aiClientFactory.CreateSpeechToTextClientAsync(speechToTextDeployment);
            await StreamTranscriptionAsync(speechToTextClient, itemId, audioChunks, audioFormat, string.IsNullOrWhiteSpace(language) ? "en-US" : language, Context.ConnectionAborted);
        }
        catch (OperationCanceledException)
        {
    ...
    private async Task RunConversationLoopAsync(string itemId, IAsyncEnumerable<string> audioChunks, string audioFormat, string speechLanguage, ISpeechToTextClient speechToTextClient, ITextToSpeechClient textToSpeechClient, string voiceName, CancellationToken cancellationToken)
    {
        var pipe = new Pipe();
        using var errorCts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
        var transcriptionTask = TranscribeConversationAsync(pipe.Reader, itemId, audioFormat, speechLanguage, speechToTextClient, textToSpeechClient, voiceName, errorCts, cancellationToken);
        try
        {
    ...
    private async Task TranscribeConversationAsync(PipeReader pipeReader, string itemId, string audioFormat, string speechLanguage, ISpeechToTextClient speechToTextClient, ITextToSpeechClient textToSpeechClient, string voiceName, CancellationTokenSource errorCts, CancellationToken cancellationToken)
    {
    ...
    private async Task ProcessConversationPromptAsync(string itemId, string prompt, ITextToSpeechClient textToSpeechClient, string voiceName, CancellationToken cancellationToken)
    {
    ...
    private async Task StreamTranscriptionAsync(ISpeechToTextClient speechToTextClient, string itemId, IAsyncEnumerable<string> audioChunks, string audioFormat, string speechLanguage, CancellationToken cancellationToken)
    {
        var pipe = new Pipe();
        using var errorCts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
        var transcriptionTask = TranscribeAudioInputAsync(itemId, pipe, audioFormat, speechLanguage, speechToTextClient, errorCts, cancellationToken);
        try
        {
            await foreach (var base64Chunk in audioChunks.WithCancellation(errorCts.Token))
            {
    ...
    private async Task TranscribeAudioInputAsync(string itemId, Pipe pipe, string audioFormat, string speechLanguage, ISpeechToTextClient speechToTextClient, CancellationTokenSource errorCts, CancellationToken cancellationToken)
    {
        try
        {
    ...
    private static string GetSpeechErrorMessage(Exception ex, string capabilityName, string fallbackMessage)
    {
        ...
        if (message.Contains("AuthenticationFailure", StringComparison.OrdinalIgnoreCase) || message.Contains("Authentication error (401)", StringComparison.OrdinalIgnoreCase) || message.Contains("check subscription information and region name", StringComparison.OrdinalIgnoreCase))
        {
