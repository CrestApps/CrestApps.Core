...
namespace CrestApps.Core.Blazor.Web.Areas.DataSources.Services;
...
public sealed class MvcAIDataSourceIndexingQueue : IMvcAIDataSourceIndexingQueue
{
    ...
    public ValueTask QueueSyncDataSourceAsync(AIDataSource dataSource, CancellationToken cancellationToken = default)
    {
        ...
        return _channel.Writer.WriteAsync(MvcAIDataSourceIndexingWorkItem.ForSyncDataSource(dataSource.Clone()), cancellationToken);
    }

    public ValueTask QueueDeleteDataSourceAsync(AIDataSource dataSource, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(dataSource);
        return _channel.Writer.WriteAsync(MvcAIDataSourceIndexingWorkItem.ForDeleteDataSource(dataSource.Clone()), cancellationToken);
    }
    ...
    {
        return QueueDocumentIdsAsync(documentIds, MvcAIDataSourceIndexingWorkItem.ForSyncSourceDocuments, cancellationToken);
    }
    ...
    {
        return QueueDocumentIdsAsync(documentIds, MvcAIDataSourceIndexingWorkItem.ForRemoveSourceDocuments, cancellationToken);
    }
