...
namespace CrestApps.Core.Blazor.Web.Components.Pages.WebCrawlers;

/// <summary>
/// Decides which records belong on the Web Crawlers pages. A record's source is either a crawl strategy or
/// an ingestion connector, and that is the only thing separating the two kinds: a strategy-backed record
/// crawls a website, while a connector-backed record reads files and is managed on the File Sources pages.
/// </summary>
public static class WebCrawlerSourceFilter
{
    ...
    /// <summary>
    /// Keeps only the records the Web Crawlers pages manage.
    /// </summary>
    /// <param name="crawlers">The records to filter.</param>
    /// <param name="strategies">The registered crawl strategies.</param>
    /// <returns>The records whose source is a registered crawl strategy.</returns>
    public static List<WebCrawler> SelectCrawlStrategyRecords(IEnumerable<WebCrawler> crawlers, IReadOnlyList<WebCrawlerStrategyDescriptor> strategies)
    {
