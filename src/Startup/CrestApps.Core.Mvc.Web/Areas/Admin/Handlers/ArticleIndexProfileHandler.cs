...
internal sealed class ArticleIndexProfileHandler : IndexProfileHandlerBase
{
    ...
    public override ValueTask<IReadOnlyCollection<SearchIndexField>> GetFieldsAsync(SearchIndexProfile indexProfile, CancellationToken cancellationToken = default)
    {
        if (!string.Equals(indexProfile.Type, IndexProfileTypes.Articles, StringComparison.OrdinalIgnoreCase))
        {
