...
public sealed class AIProfileController : Controller
{
    ...
    private readonly ICatalog<AIToolDefinition> _toolInstanceCatalog;
    ...
    public AIProfileController(
        ...
        ICatalog<AIToolDefinition> toolInstanceCatalog,
    ...
    public async Task<IActionResult> Create(AIProfileViewModel model, List<IFormFile> Documents)
    {
        ...
        model.SelectedToolDefinitionIds = await GetValidToolDefinitionIdsAsync(model.SelectedToolDefinitionIds);
    ...
    private async Task PopulateDropdownsAsync(AIProfileViewModel model)
    {
        ...
        var toolDefinitions = await _toolInstanceCatalog.GetAllAsync();
        var selectedToolDefinitionIds = new HashSet<string>(model.SelectedToolDefinitionIds ?? [], StringComparer.Ordinal);
        model.AvailableToolDefinitions = toolDefinitions.OrderBy(i => i.DisplayText, StringComparer.OrdinalIgnoreCase).Select(i => new AIToolDefinitionSelectionItem { ItemId = i.ItemId, DisplayText = i.DisplayText, Description = i.Description, Source = i.Source, IsSelected = selectedToolDefinitionIds.Contains(i.ItemId), }).ToList();
    ...
    private async Task<string[]> GetValidToolDefinitionIdsAsync(IEnumerable<string> selectedIds)
    {
