...
{
    private readonly Channel<MvcAIChatDocumentIndexingWorkItem> _channel = Channel.CreateUnbounded<MvcAIChatDocumentIndexingWorkItem>(new UnboundedChannelOptions { SingleReader = true, SingleWriter = false, });
    ...
    public ValueTask QueueDeleteChunksAsync(IReadOnlyCollection<string> chunkIds, CancellationToken cancellationToken = default)
    {
        ...
        return _channel.Writer.WriteAsync(MvcAIChatDocumentIndexingWorkItem.ForDeleteChunks(chunkIds.Where(id => !string.IsNullOrWhiteSpace(id)).Distinct(StringComparer.Ordinal).ToArray()), cancellationToken);
    }
