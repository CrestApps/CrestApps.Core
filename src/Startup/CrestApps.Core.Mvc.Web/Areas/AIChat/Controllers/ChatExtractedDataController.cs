...
public sealed class ChatExtractedDataController : Controller
{
    ...
    {
        return records.Select(record => new ChatExtractedDataRowViewModel { SessionId = record.SessionId, SessionStartedUtc = record.SessionStartedUtc, Values = record.Values.ToDictionary(pair => pair.Key, pair => string.Join(", ", pair.Value), StringComparer.OrdinalIgnoreCase), }).ToList();
    }

    private static string GenerateCsv(IReadOnlyList<ChatExtractedDataRowViewModel> rows, IReadOnlyList<string> columns)
    {
