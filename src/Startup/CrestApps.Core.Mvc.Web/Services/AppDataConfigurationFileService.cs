...
public sealed class AppDataConfigurationFileService
{
    ...
    private static void SetSectionNode(JsonObject root, string sectionPath, JsonNode value)
    {
        ...
        for (var i = 0; i < segments.Length - 1; i++)
        {
            ...
            var propertyName = TryGetPropertyName(current, segment, out var existingName) ? existingName : segment;
    ...
    private static JsonNode GetSectionNode(JsonObject root, string sectionPath)
    {
        ...
        {
            if (current is not JsonObject currentObject || !TryGetPropertyName(currentObject, segment, out var propertyName))
            {
                return null;
            }
    ...
    {
        return sectionPath.Split(':', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
    }
