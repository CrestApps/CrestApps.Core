...
/// <summary>
/// Represents a user-configured, reusable instance of an <see cref="IAIToolInstanceDefinition"/>.
/// Unlike a plain <c>AITool</c> whose arguments are always supplied by the model, a tool instance
...
/// <see cref="IAIToolInstanceDefinition"/>. The same definition may be instantiated multiple times,
/// each with different settings and a distinct <see cref="Description"/> so the model can tell the
/// instances apart.
/// </remarks>
public sealed class AIToolInstance : SourceCatalogEntry, IDisplayTextAwareModel, IModifiedUtcAwareModel, ICloneable<AIToolInstance>
{
    /// <summary>
    ...
    /// signal the model uses to distinguish between multiple instances of the same definition, so it
