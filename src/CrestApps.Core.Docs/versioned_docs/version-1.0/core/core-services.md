...
Every feature in the framework shares common concerns like OData filter validation and base service patterns. `AddCoreServices()` registers these shared services so that higher-level features can depend on them without duplication.
