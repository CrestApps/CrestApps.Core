...
When the no-tools structured output path receives a successful (HTTP 200) response that cannot be parsed into structured task results — for example an empty completion, a safety refusal, a truncated response, or free-form prose instead of JSON — the framework now records an explicit `Failed` result with a diagnostic message and logs the unparseable response at `Warning` (including a preview of the raw AI response). Previously this was swallowed as a missing result, which caused the task to consume every retry attempt before the whole post-close cycle was marked failed. Additionally, the framework no longer issues a post-session AI request when there is no meaningful user content to evaluate; sessions whose user prompts are empty, whitespace-only, or system-generated are skipped so a real AI call is only made when there is something to analyze.
