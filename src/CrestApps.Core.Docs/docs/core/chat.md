...
Profile-level post-session configuration also accepts `AIProfilePostSessionSettings.ToolInstanceNames`, so parameterized AI tool instances can be invoked during post-session analysis alongside regular tools. Configuring only tool instances is enough to enable the tool-driven post-session path; no regular tool names are required.
