...
(function (window, document) {
  ...
  function attach(opts) {
    ...
    var webRtcIceServers = Array.isArray(opts.webRtcIceServers) && opts.webRtcIceServers.length ? opts.webRtcIceServers : [{
    ...
    var REALTIME_GATE_OPEN_LEVEL = 0.05,
      REALTIME_GATE_ECHO_MARGIN = 0.08,
      REALTIME_GATE_HANGOVER_MS = 1000;
    ...
    function updateRealtimeButton() {
      ...
      if (isRealtimeActive) {
        ...
        if (endHtml) {
          realtimeBtn.replaceChildren(window.DOMPurify.sanitize(endHtml, {
        ...
        if (startHtml) {
          realtimeBtn.replaceChildren(window.DOMPurify.sanitize(startHtml, {
            RETURN_DOM_FRAGMENT: true
          }));
        }
      }
      ...
      try {
        ...
        var _loop = function loop() {
          ...
          if (micLevel > threshold) {
            speakingUntilMs = nowMs + REALTIME_GATE_HANGOVER_MS;
          }
          ...
          if (wasAssistantSpeaking && !assistantSpeaking) {
            listenGraceUntilMs = nowMs + REALTIME_GATE_LISTEN_GRACE_MS;
          }
    ...
    function stopWebRtcEchoGuard() {
      if (realtimeWebRtcMonitorRaf) {
        try {
          window.cancelAnimationFrame(realtimeWebRtcMonitorRaf);
        } catch (e) {}
        realtimeWebRtcMonitorRaf = 0;
      }
      if (realtimeWebRtcMonitorCtx) {
